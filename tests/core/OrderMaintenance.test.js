...
test("Should maintain order when added sequentially", () => {
	...
	const nodes = [];
	for (let i = 0; i < 10000; ++i) nodes.push(orderMaintenance.addAfter(orderMaintenance.tail));
...
test("Should maintain order when added in reserve order", () => {
	const orderMaintenance = new OrderMaintenance();
	const nodes = [];
	for (let i = 0; i < 10000; ++i) nodes.push(orderMaintenance.addAfter(orderMaintenance.head));
	for (let i = 1; i < 10000; ++i) assert(nodes[i - 1].tag > nodes[i].tag);
});
...
test("Should maintain order when added to the middle", () => {
	const orderMaintenance = new OrderMaintenance();
	const originalNodes = [];
	for (let i = 0; i < 1000; ++i) originalNodes.push(orderMaintenance.addAfter(orderMaintenance.tail));
	const newNodes = [];
	for (let i = 10; i <= 1000; i += 10) newNodes.push(
		...originalNodes.slice(i - 10, i),
		orderMaintenance.addAfter(originalNodes[i - 1])
	);
